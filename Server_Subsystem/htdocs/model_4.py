...
def main():
    ...
    cap = cv2.VideoCapture("files/" + input_file)
    ...
    if np.count_nonzero(key_point) == 0:
        ...
        create_txt(input_file, output_dir, "Error: Invalid Video (Can't detect signer)")
        return
    elif key_point is None:
        print("null keypoints")
        create_txt(input_file, output_dir, "Error: No keypoints generated.")
        return

    if key_point is not None:

        # model = load_model("models/model11/best_model_epoch_33.h5")
        model = load_model("models/best_model_epoch_70.h5")
        ...
        if predicted_prob < confidence_threshold:
            ...
            create_txt(input_file, output_dir, f"Not recognized. Please try again.")
        else:
